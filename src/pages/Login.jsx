...
const FormInput = ({ label, id, error, icon: Icon, children }) => (
  <div className="flex flex-col gap-2 group">
    <label htmlFor={id} className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-1 group-focus-within:text-accent transition-colors">
      {label}
    </label>
    ...
    {error && (
      <span role="alert" className="text-[11px] text-red-500 font-bold px-1 animate-in fade-in slide-in-from-top-1">
        {error.message}
      </span>
    )}
  </div>
...
const Login = () => {
  ...
  const { mutate: loginUser, isPending: isLoggingIn, reset: resetLoginMutation } = useLogin();
        ...
        <div className="w-full max-w-[400px] animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-100">
          <div className="mb-12">
            ...
            <p className="text-text-muted font-medium leading-relaxed">
              Enter your credentials to access the <span className="text-accent font-bold">FixPay</span> administrative terminal.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8" noValidate>
            ...
            <div className="space-y-6">
              ...
              <FormInput label="Security Key" id="password" error={errors.password} icon={Lock}>
                <div className="relative">
                  <input
                    id="password"
